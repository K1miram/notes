...
public class NotebookItem extends Item {
    ...
    public NotebookItem(Properties properties, Identifier id) {
        super(properties);
        backgroundTextureId = id;
    }

    @Override
    public @NotNull InteractionResult use(@NonNull Level level, Player player, @NotNull InteractionHand usedHand) {
        ItemStack stack = player.getItemInHand(usedHand);
        ItemStack stackInOffhand = player.getItemInHand(InteractionHand.OFF_HAND);
        if (stackInOffhand.is(ModItems.FINALIZED_NOTE)) {
            ...
            if (pages.size() < 100) {
                ...
                player.setItemInHand(InteractionHand.OFF_HAND, stackInOffhand.copyWithCount(stackInOffhand.count() - 1));
