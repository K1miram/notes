...
public record NoteContent(String text, List<Image> images) {
    public static final Codec<NoteContent> CODEC = RecordCodecBuilder.create(builder -> builder.group(
            Codec.STRING.fieldOf("text").orElse("").forGetter(NoteContent::text),
            Image.CODEC.listOf().fieldOf("images").orElse(Collections.emptyList()).forGetter(NoteContent::images)
